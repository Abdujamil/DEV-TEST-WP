		...
		function get_ajax_query( $options = array() ) {
			...
			foreach ( array_keys( $groups ) as $group_title ) {
				...
				foreach ( array_keys( $posts ) as $post_id ) {
                    ...
                    if ($field['name'] === 'catalog_books_list'){
                        $author_names = [];
                        $authors = get_field('book_author', $post_id);
                        if (!empty($authors)){
                            foreach ($authors as $author) {
                                array_push($author_names, $author->post_title);
                            }
                        }

                        $string = !empty($author_names) ? ' (' . implode(', ', $author_names) . ')' : '';
                        $posts[ $post_id ] = $this->get_post_title( $posts[ $post_id ], $field, $options['post_id'] ) . $string;
                    }else{
			...
			<ul class="acf-bl list values-list">
				...
				if ( ! empty( $field['value'] ) ) :
					...
					foreach ( $posts as $post ) :
                        $author_names = [];
                        $authors = get_field('book_author', $post->ID);
                        if (!empty($authors)) {
                            foreach ($authors as $author) {
                                array_push($author_names, $author->post_title);
                            }
					...
					<li>
						...
						<span data-id="<?php echo esc_attr( $post->ID ); ?>" class="acf-rel-item">
								<?php echo acf_esc_html( $this->get_post_title( $post, $field ) ) . $string; ?>
